...
void triggerSetup_missingTooth(void)
{
  ...
  if ((configPage4.TrigSpeed == CRANK_SPEED) && ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) || (configPage2.injLayout == INJ_SEQUENTIAL) || (configPage6.vvtEnabled > 0)))
  {
...
void triggerPri_missingTooth(void)
{
  ...
  if (curGap >= triggerFilterTime) // Pulses should never be less than triggerFilterTime, so if they are it means a false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
  {
    ...
    if ((toothLastToothTime > 0) && (toothLastMinusOneToothTime > 0))
    {
      ...
      if (configPage10.PollLevelTeeth > 1 && configPage4.trigPatternSec == SEC_TRIGGER_POLL && currentStatus.hasSync == false || currentStatus.RPM < 2000)
      {
        if (toothCurrentCount == configPage10.PollLevelTeeth && configPage4.PollLevelPolarity == READ_SEC_TRIGGER())
          ? revolutionOne = 1 : revolutionOne = 0; // at selected tooth check if the cam sensor is high or low in poll level mode
      }
      /*
...
void triggerSetEndTeeth_missingTooth(void)
{
  ...
  if (((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) || (configPage4.sparkMode == IGN_MODE_SINGLE)) && (configPage4.TrigSpeed == CRANK_SPEED) && (configPage2.strokes == FOUR_STROKE))
  {
...
uint16_t getRPM_BasicDistributor(void)
{
  ...
  {
    distributorSpeed = CRANK_SPEED;
  } // For 2 stroke distributors, the tooth rate is based on crank speed, not 'cam'
...
void triggerPri_HondaJ32(void)
{
  ...
  {
    if (curGap < (lastGap >> 1) * 3 || lastGap == 0)
    {                      // Regular tooth, lastGap == 0 at startup
      toothCurrentCount++; // Increment teeth between gaps
      lastGap = curGap;
    }
    else
    { // First tooth after the missing tooth
      if (toothCurrentCount == 15)
      { // 15 teeth since the gap before this, meaning we just passed the second gap and are synced
        currentStatus.hasSync = true;
  ...
  if ((curGap >= triggerFilterTime) || (currentStatus.startRevolutions == 0))
  {
    ...
    if (currentStatus.hasSync == true)
    {
      ...
      if ((configPage4.triggerFilter == 1) || (currentStatus.RPM < 1400))
      {
        ...
        {
          triggerToothAngle = 110;
          triggerFilterTime = rshift<3>(curGap * 3UL);
      ...
      else if (configPage4.triggerFilter == 3)
      {
        ...
        {
          triggerToothAngle = 70;
          triggerFilterTime = rshift<3>(curGap * 11UL);
        } // 96.26 degrees with a target of 110
        else
        {
          triggerToothAngle = 110;
          triggerFilterTime = rshift<5>(curGap * 9UL);
...
static void triggerRoverMEMSCommon(void)
{
  ...
  else
  {
    currentStatus.hasSync = false;
    BIT_SET(currentStatus.status3, BIT_STATUS3_HALFSYNC);
  } // If nothing is using sequential, we  set half sync bit
